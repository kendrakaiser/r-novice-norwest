...
Once you've designed an experiment and collected
...
the NorWest stream temperature dataset from the Middle Snake River. 

By the end of these lessons you will be able to do read in the data, and plot the stream temperatures for an individual reach over time!
...
file. The gist: you type in commands, R tries to execute them, and then
returns a result.
...
When using R as a calculator, the order of operations is the same as normal.
...
re thrown when R cannot proceed with a calculation. Warnings on the
