...
them using the `$` operator, here we use `[]` to index into the column to show 
...
But coercion can also be very useful! For example, in our `temps` data
`QAQC` is numeric, but we know that the 1s and 0s actually represent
...
```{r}
temps$QAQC[1:5]
temps$QAQC <- as.logical(temps$QAQC)
temps$QAQC[1:10]
```
